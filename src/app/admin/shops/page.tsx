...
export default async function AdminShopsPage({ searchParams }: AdminShopsPageProps) {
  ...
  return (
    <AppShell
      title="公共商店管理"
      badge="商店维护"
      description="管理员可在此直接维护商店条目。所有创建与修改动作均需消耗一次性密码，并同步写入审计日志。"
    >
        ...
        <article className="panel rounded-[28px] p-6">
          <div className="grid gap-3 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border border-[var(--border-soft)] bg-[rgba(255,250,241,0.82)] px-4 py-3">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--color-ink-900)]">
                创建、编辑与启停条目都会记录为 <code>SHOP_ITEM_UPDATED</code>。
              </p>
            </div>
          </div>
        ...
        {shops.map((shop) => (
          <section key={shop.id} className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
              ...
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
                    结算货币：{formatCurrency(shop.currency)}。当前共 {shop.items.length} 个条目。
                  </p>
                  {shop.description ? (
                  ...
                  >
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">商品名称</label>
                        <input
                          name="name"
                          type="text"
                          required
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">分类</label>
                        <input
                          name="category"
                          type="text"
                          required
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">价格</label>
                        <input
                          name="price"
                          type="number"
                          min={0}
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">排序</label>
                        <input
                          name="sortOrder"
                          type="number"
                          min={0}
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">来源</label>
                        <input
                          name="importedSource"
                          type="text"
                          maxLength={240}
                          defaultValue={item.importedSource ?? ""}
                          placeholder="例如：SRD / 西征设定"
                          className="focus-ring w-full rounded-2xl border border-[var(--border-strong)] bg-[rgba(255,250,241,0.95)] px-4 py-3 text-sm text-[var(--color-ink-900)]"
                        />
                      </div>

                      <div className="space-y-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">状态</label>
                        <select
                          name="isActive"
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">描述</label>
                        <textarea
                          name="description"
                          rows={3}
                          maxLength={240}
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <label className="block text-sm font-semibold text-[var(--color-ink-700)]">一次性密码</label>
                        <input
                          name="otpCode"
                          type="text"
                          required
                    ...
                    <div className="mt-4 flex flex-col gap-3 border-t border-[var(--border-soft)] pt-4 sm:flex-row sm:items-center sm:justify-between">
                      <p className="text-sm leading-6 text-[var(--muted)]">
                        当前状态：{item.isActive ? "启用中" : "已停用"}。保存后将同步更新条目与审计记录。
                      </p>
                      <button
                        type="submit"
            ...
            <article className="panel rounded-[28px] p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
                新增条目默认启用。创建动作同样需要消耗一次性密码，并写入审计日志。
              </p>
